...
foreach my$sample (nsort keys %alleles) {
	...
	#print $min_sum, "\t", scalar(@min_alts), "\n" ;

#	##skip to next sample if @min_alts is empty ==> turned off and if(scalar@min_alts>0) added so that low coverage sample stats print
	#if (@min_alts == 0) {next ;}

	my$min_mean = 0 ;
	if (scalar@min_alts>0){$min_mean=$min_sum/(scalar@min_alts)} ;

	my$min_dev = 0 ;
	my$min_stdev = 0 ;
	if (scalar@min_alts>0) {
		foreach (@min_alts) {$min_dev += ($_ - $min_mean)**2 ;}
		$min_stdev = sqrt($min_dev / (scalar@min_alts)) ;
	}
	##########################################################################################
	...
	$AC_output =~ s/.gz// ;
	my$invar_output = $AC_output ;
	$AC_output =~ s/.pileup/${sample}_${scaff_keyword}_all_AF_counts.txt/ ;
	$invar_output =~ s/.pileup/${sample}_${scaff_keyword}_all_AF_invar_counts.txt/ ;

	open OUT, ">$AC_output" or die "cannot open $AC_output\n" ;
	open IOUT, ">$invar_output" or die "cannot open $invar_output\n" ;

	my$total_S = 0 ;
	my$snps = 0 ;
	my$indels = 0 ;
	my$total_fixed_nonref = 0 ;
	...
	foreach my$scaffold (nsort keys %{$alleles{$sample}}) {
		foreach my$position (sort {$a<=>$b} keys %{$alleles{$sample}{$scaffold}}) {
			...
			if ($ref_count > 0) {
				if (scalar(@alt_bp) == 0) {
					print IOUT $scaffold, "\t", $position, "\t", $ref_base, "\t", $ref_count, "\t", "NA", "\t", "NA", "\t", "NA", "\n" ;
					$total_fixed_nonref ++ ;
				} # invariant site
			...
			## if none of the alleles are the reference
				...
				if (scalar(@alt_bp) < 2) {
					print IOUT $scaffold, "\t", $position, "\t", $ref_base, "\t0\t", $alt_bp[0], "\t", $alts{$alt_bp[0]}, "\t", "1", "\n" ;
					$total_fixed_nonref ++ ;
					next ;
				} # invariant site
				elsif (scalar(@alt_bp) > 2) {print $sample, "\t", $scaffold, "\t", $position, "\t", "more than one alternate allele\n" ; next ;}
				else {
					print OUT $scaffold, "\t", $position, "\t", $ref_base, "\t0\t", $alt_bp[0], ";", $alt_bp[1], "\t", $alts{$alt_bp[0]}, ";", $alts{$alt_bp[1]}, "\t", $alts{$alt_bp[1]}/($alts{$alt_bp[1]}+$alts{$alt_bp[0]}), "\n" ;
...
sub parse_pileup {
	...
	while (<$input>) {
		...
		foreach my$column (3..$#split) {
			...
			##read alignment
				...
				#print "calculated minimum alternate allele depth: ", $min_alt, "\n" ;
				...
				## record low coverage sites by min alt (these will be removed in the subsequent steps, but I don't want to multiply count the position)
				if ($depth < $min_alt) {$low_depth ++ ;}
	...
	#close PILEUP ;
	print "Number of low depth sites removed: ", $low_depth, "\n" ;
